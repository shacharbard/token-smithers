...
case "$filename" in
  screenshot*|Screenshot*|"Screen Shot"*|"screen shot"*)
    echo "REDIRECT: This looks like a screenshot. Resize before reading to save tokens: sips --resampleWidth 512 ${safe_path} --out /tmp/resized.png && Read /tmp/resized.png" >&2
    exit 2 ;;
esac
...
if [[ -f "$file_path" ]]; then
  ...
  if [[ "$file_size" -gt 512000 ]]; then
    echo "REDIRECT: Image is ${file_size} bytes. Resize before reading to save tokens: sips --resampleWidth 512 ${safe_path} --out /tmp/resized.png && Read /tmp/resized.png" >&2
    exit 2
  fi
fi
